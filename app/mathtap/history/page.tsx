    ...
    return (
        <div className="mt-3">
            <button
                ...
                className="flex items-center justify-between w-full text-sm text-blue-700 hover:text-blue-800 bg-blue-50 hover:bg-blue-100 px-4 py-3 rounded-xl border border-blue-200 transition-all duration-200 font-medium"
            >
            ...
            <div className={`overflow-hidden transition-all duration-500 ease-in-out ${isOpen ? 'max-h-[800px] opacity-100 mt-3' : 'max-h-0 opacity-0 mt-0'
                ...
                <div className={`grid grid-cols-1 md:grid-cols-2 gap-3 transition-transform duration-500 ease-out ${isOpen ? 'translate-y-0' : '-translate-y-4'
                    ...
                    {words.map((w: IncorrectWord, index) => (
                        <div
                            key={`${w.word_number}-${index}`}
                            className={`bg-blue-50 p-4 rounded-xl border border-blue-200 transition-all duration-300 ease-out hover:shadow-sm ${isOpen ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-2'
                                }`}
                            style={{
                        ...
                        >
                            <div className="flex items-start space-x-3">
                                <span className="text-xs text-blue-600 bg-blue-100 px-2 py-1 rounded-lg font-medium flex-shrink-0">
                                    No.{w.word_number}
                                </span>
                                <div className="flex-1">
                                    <div className="font-bold text-lg text-gray-800 mb-1" dangerouslySetInnerHTML={{ __html: renderMath(w.word) }} />
                                    <div className="text-sm text-gray-600 leading-relaxed font-mono">{w.meaning}</div>
                                </div>
                            </div>
                        </div>
                    ))}
            ...
            try {
                ...
                if (error) {
                    ...
                    resultsData.forEach(result => {
                        // "Mathtap: 中1" -> "中1" のように表示用名を整形
                        const textName = result.selected_text ? result.selected_text.replace('Mathtap: ', '') : '計算テスト';
        ...
        <main className="min-h-screen bg-gray-50">
            <Background className="min-h-screen">
                <div className="max-w-5xl mx-auto px-4 py-8" style={{ marginTop: '25px' }}>
                    ...
                    {error && (
                        <div className="bg-blue-50 border border-blue-200 text-blue-700 px-6 py-4 rounded-2xl mb-8">
                            {error}
                        </div>
                    )}
                    ...
                    {!loading && results.length === 0 && (
                        <div className="flex flex-col items-center justify-center py-16 bg-white/80 backdrop-blur-xl rounded-3xl border border-white/50 shadow-xl text-center px-4">
                            ...
                            <button
                                ...
                                className="bg-blue-600 hover:bg-blue-700 text-white px-8 py-4 rounded-xl font-bold text-lg transition-all shadow-lg shadow-blue-200 hover:shadow-blue-300 transform hover:-translate-y-0.5 flex items-center gap-2"
                            >
                                                    ...
                                                    <div key={result.id} className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm hover:shadow-md transition-all duration-300">
                                                        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4">
                                                            <div className="flex items-center gap-4">
                                                                <div className={`w-14 h-14 rounded-2xl flex items-center justify-center text-xl font-bold flex-shrink-0 ${isHighScore ? 'bg-blue-50 text-blue-600' : 'bg-gray-50 text-gray-600'
                                                                    }`}>
                                                                ...
                                                                <div>
                                                                    ...
                                                                    <div className="font-medium text-gray-900">
                                                                        <span className="bg-gray-100 px-2 py-1 rounded-lg text-sm mr-2">
                                                                            {result.unit || '範囲不明'}
                                                                        </span>
