...
export default function ActionButtons({
  ...
  const executeShare = useCallback(async (targetId: string, suffix: string, ratio: number) => {
    try {
      ...
      const filename = `edulens_university_${slug}_${year}_${suffix}.png`;
      ...
      if (navigator.share) {
        try {
          await navigator.share({
            ...
            text: `${eventName}まで、あと${diffDays}日！ #大学入試 #共通テスト #カウントダウン #EduLens`,
          });
          return;
  ...
  }, [eventName, slug, year, diffDays]);

  return (
    <>
      <div className="mt-8 flex flex-col items-center gap-4 w-full max-w-xs mx-auto mb-12">
        {/* 画像で共有 (Modal Trigger) */}
        <button
          type="button"
        ...
        {/* Xで共有 (テキストリンク) */}
        <a
          href={`https://twitter.com/intent/tweet?text=${encodeURIComponent(`${eventName}まで、あと${diffDays}日！ #大学入試 #共通テスト #カウントダウン`)}&url=${encodeURIComponent(`https://edulens.jp/countdown/university/${slug}/${year}`)}`}
          target="_blank"
