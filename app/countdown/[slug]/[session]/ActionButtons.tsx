...
export default function ActionButtons({
  ...
  const executeShare = useCallback(async (targetId: string, suffix: string, ratio: number) => {
    try {
      ...
      const filename = `edulens_qual_${slug}_${sessionSlug}_${suffix}.png`;
      ...
      if (navigator.share) {
        try {
          await navigator.share({
            ...
            text: `${examName} ${sessionName}まで、あと${diffDays}日！ #資格試験 #カウントダウン #EduLens`,
          });
          return;
  ...
  }, [examName, sessionName, slug, sessionSlug, diffDays]);
