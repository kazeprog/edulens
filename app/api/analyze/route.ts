...
const analyzeSchema = z.object({
    ...
    level: z.enum(["3", "pre-2", "pre-2-plus", "2", "pre-1", "1"]),
    problemType: z.enum(["opinion", "summary", "email"]).optional().default("opinion"),
});
...
export async function POST(req: NextRequest) {
    try {
        ...
        const { images, level, problemType } = result.data;
        const eikenLevelStr = EikenLevelsMap[level] || `${level}級`; // e.g. "準2級"
