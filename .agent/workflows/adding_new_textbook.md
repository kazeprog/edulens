...
3.  **Schema Consistency Check**:
    -   If the JSON is generated by a script, verify that the script and the generated JSON use the exact same field names expected by Mistap.
    -   The test creation flow filters by `word_number`, so do not use alternative keys such as `wordNumber`.
    -   After generating the JSON, spot-check the first few entries and confirm the numbering field is really `word_number`.
4.  **Range Smoke Test Before UI Work**:
    -   Before wiring the textbook into the UI, run a quick local check that a basic range such as `1-100` returns data from the JSON.
    -   This catches schema mismatches early, before they surface as a browser-side "Range not found" alert.
